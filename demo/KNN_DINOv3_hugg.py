...
from emcfsys.utils.classification_tasks import HEAD_KNN  # noqa: E402
...
class HuggingFaceDINOv3FeatureExtractor(nn.Module):
    """HuggingFace DINOv3 feature extractor for KNN classification."""
    ...
    def __init__(
        ...
        super().__init__()
        if feature_type not in {"pooler", "cls", "patch_mean", "cls_patch_mean"}:
        ...
        try:
            ...
            raise ImportError(
                "KNN_DINOv3.py requires the HuggingFace transformers package. "
        ...
        self.num_register_tokens = int(
            getattr(
                self.model.config,
                "num_register_tokens",
                4 if "dinov3" in model_name.lower() else 0,
            )
        )
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        if self.feature_type == "pooler":
            pooler_output = getattr(outputs, "pooler_output", None)
            return cls_token if pooler_output is None else pooler_output
...
def build_image_transform(image_processor, img_size: int):
    mean = tuple(getattr(image_processor, "image_mean", DEFAULT_MEAN))
    std = tuple(getattr(image_processor, "image_std", DEFAULT_STD))
    return A.Compose(
        [
...
def fit_knn(
    *,
    feature_extractor: HuggingFaceDINOv3FeatureExtractor,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "KNN classification demo with a HuggingFace DINOv3 ViT-B/16 backbone. "
    ...
    parser.add_argument(
        "--feature-type",
        choices=("pooler", "cls", "patch_mean", "cls_patch_mean"),
        default="pooler",
    )
    ...
    parser.add_argument(
        ...
        help="Optional HuggingFace token. Defaults to the HF_TOKEN environment variable.",
    )
    ...
    transform = build_image_transform(image_processor, args.img_size)
    ...
    print(f"HuggingFace DINOv3 model: {args.model_name}")
    print(f"Feature type: {args.feature_type}")
    print(f"Device: {device}")
    print("KNN uses deterministic resize + DINOv3 image processor normalization.")
    ...
    checkpoint_path = args.save_dir / "classification_knn_dinov3_vitb16_final.pth"
        ...
        json.dumps(
            {
                "task": "classification",
                "head_name": HEAD_KNN,
                "feature_backbone": "huggingface_dinov3_vitb16",
                ...
                "device": str(device),
                "class_names": class_names,
                "image_mean": getattr(image_processor, "image_mean", DEFAULT_MEAN),
                "image_std": getattr(image_processor, "image_std", DEFAULT_STD),
